...
function editPasswordEvent() {
    ...
    editButtons.forEach((button) => {
        button.onclick = () => {
            ...
            let id = parentDiv.id;
            let email = parentDiv
                .querySelector('.password-email')
                .innerHTML.trim();
            let username = parentDiv
                .querySelector('.password-username')
                .innerHTML.trim();
            ...
            let modalMPW = localStorage.getItem('key'); //modal.querySelector(".edit-master-password");
            ...
            document.querySelector('#edit-modal-save').onclick = () => {
                ...
                if (modalPassword.value !== '')
                    data.encrypted_password = encrypt(
                        modalPassword.value,
                        modalMPW
                    );
                ...
                if (!flag) {
                    ...
                    else data.tags = modalTagsList;
                }

                // update password
                patchData(
                    `${window.location.origin}/api/passwords/` + id,
                    (data = data)
                )
                    .then((response) => response.json())
                    .then(() => {
                        // display passwords
                        getPasswords().then((results) =>
